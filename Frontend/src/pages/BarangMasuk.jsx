...
import { Plus, Printer, Edit, Trash2, Save, X, CheckCircle, Truck } from 'lucide-react';
import api from '../api';
...
const BarangMasuk = () => {
  ...
  const handleProcessSave = async () => {
      try {
          if (isEdit) {
            await api.put(`/transactions-in/${currentId}/`, formData);
            triggerToast("Transaksi berhasil diperbarui!");
          } else {
            await api.post('/transactions-in/', formData);
            triggerToast("Barang masuk berhasil dicatat!");
          }
          fetchData();
  ...
  return (
    <DashboardLayout>
        <h2 className="text-2xl font-bold text-black mb-6">Barang Masuk</h2>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
            ...
            <button onClick={() => setShowModalCetak(true)} className="bg-[#10b981] hover:bg-green-600 text-white font-medium py-2 px-4 rounded shadow-sm flex items-center gap-2 transition-colors">
                <Printer className="w-4 h-4" /> Cetak Barang Masuk
            </button>
        </div>
                        ...
                        {loading ? (
                            ...
                            currentData.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="border border-gray-300 px-4 py-2 font-bold text-green-600">+{item.quantity} Kg</td>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 transition-opacity">
                <div className="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4 transform transition-all animate-fade-in-down">
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-bold text-black mb-1">Jumlah Masuk (Kg)</label>
                            <Input type="number" name="quantity" value={formData.quantity} onChange={handleChange} className="w-full border-gray-300" />
                        </div>
                        <div>
        ...
        {showConfirmModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
                <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-4 transform transition-all animate-fade-in-down">
                    ...
                    <div className="p-6">
                        <p className="text-sm text-gray-600 mb-4">Silakan periksa kembali data barang masuk berikut.</p>
        ...
        {showModalHapus && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 transition-opacity">
                <div className="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4 transform transition-all animate-fade-in-down">
                    ...
                    <div className="p-6">
                        <p className="text-black font-bold text-base">Apakah anda yakin? Stok barang akan otomatis dikurangi kembali.</p>
                    </div>
                    <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-300 bg-white rounded-b-lg">
